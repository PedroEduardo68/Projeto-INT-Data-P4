...
__forceinline static int handle_tier_0(__lmem uint32_t *parrep, PIF_PKT_INFO_TYPE struct pif_pkt_info *pktinfo, int *pkt_byteoff, uint32_t pkt_min_off)
{
    ...
    if (PIF_PARREP_T0_VALID(ctldata)) {
        curr_len = PIF_PARREP_report_ethernet_LEN_B;
    }
...
__forceinline static int handle_tier_1(__lmem uint32_t *parrep, PIF_PKT_INFO_TYPE struct pif_pkt_info *pktinfo, int *pkt_byteoff, uint32_t pkt_min_off)
{
    ...
    if (PIF_PARREP_T1_VALID(ctldata)) {
        curr_len = PIF_PARREP_report_ipv4_LEN_B;
    }
...
__forceinline static int handle_tier_2(__lmem uint32_t *parrep, PIF_PKT_INFO_TYPE struct pif_pkt_info *pktinfo, int *pkt_byteoff, uint32_t pkt_min_off)
{
    ...
    if (PIF_PARREP_T2_VALID(ctldata)) {
        curr_len = PIF_PARREP_report_udp_LEN_B;
    }
...
__forceinline static int handle_tier_3(__lmem uint32_t *parrep, PIF_PKT_INFO_TYPE struct pif_pkt_info *pktinfo, int *pkt_byteoff, uint32_t pkt_min_off)
{
    ...
    if (PIF_PARREP_T3_VALID(ctldata)) {
        curr_len = PIF_PARREP_report_LEN_B;
    }
...
__forceinline static int handle_tier_14(__lmem uint32_t *parrep, PIF_PKT_INFO_TYPE struct pif_pkt_info *pktinfo, int *pkt_byteoff, uint32_t pkt_min_off)
{
    __lmem struct pif_parrep_ctldata *ctldata = (__lmem struct pif_parrep_ctldata *)(parrep + PIF_PARREP_CTLDATA_OFF_LW);
    uint32_t orig_len = PIF_PARREP_T14_ORIG_LEN(ctldata);
    uint32_t curr_len = 0;

    if (!PIF_PARREP_T14_DIRTY(ctldata)) {
        *pkt_byteoff -= orig_len;
        if (*pkt_byteoff < 0)
            return -PIF_DEPARSE_ERROR_BUFFER_OVERRUN;
    ...
    if (PIF_PARREP_T14_VALID(ctldata)) {
...
__forceinline static int handle_tier_15(__lmem uint32_t *parrep, PIF_PKT_INFO_TYPE struct pif_pkt_info *pktinfo, int *pkt_byteoff, uint32_t pkt_min_off)
{
    __lmem struct pif_parrep_ctldata *ctldata = (__lmem struct pif_parrep_ctldata *)(parrep + PIF_PARREP_CTLDATA_OFF_LW);
    uint32_t orig_len = PIF_PARREP_T15_ORIG_LEN(ctldata);
    uint32_t curr_len = 0;

    if (!PIF_PARREP_T15_DIRTY(ctldata)) {
        *pkt_byteoff -= orig_len;
        if (*pkt_byteoff < 0)
            return -PIF_DEPARSE_ERROR_BUFFER_OVERRUN;
    ...
    if (PIF_PARREP_T15_VALID(ctldata)) {
...
__forceinline static int handle_tier_16(__lmem uint32_t *parrep, PIF_PKT_INFO_TYPE struct pif_pkt_info *pktinfo, int *pkt_byteoff, uint32_t pkt_min_off)
{
    __lmem struct pif_parrep_ctldata *ctldata = (__lmem struct pif_parrep_ctldata *)(parrep + PIF_PARREP_CTLDATA_OFF_LW);
    uint32_t orig_len = PIF_PARREP_T16_ORIG_LEN(ctldata);
    uint32_t curr_len = 0;

    if (!PIF_PARREP_T16_DIRTY(ctldata)) {
        *pkt_byteoff -= orig_len;
        if (*pkt_byteoff < 0)
            return -PIF_DEPARSE_ERROR_BUFFER_OVERRUN;
    ...
    if (PIF_PARREP_T16_VALID(ctldata)) {
...
__forceinline static int handle_tier_17(__lmem uint32_t *parrep, PIF_PKT_INFO_TYPE struct pif_pkt_info *pktinfo, int *pkt_byteoff, uint32_t pkt_min_off)
{
    __lmem struct pif_parrep_ctldata *ctldata = (__lmem struct pif_parrep_ctldata *)(parrep + PIF_PARREP_CTLDATA_OFF_LW);
    uint32_t orig_len = PIF_PARREP_T17_ORIG_LEN(ctldata);
    uint32_t curr_len = 0;

    if (!PIF_PARREP_T17_DIRTY(ctldata)) {
        *pkt_byteoff -= orig_len;
        if (*pkt_byteoff < 0)
            return -PIF_DEPARSE_ERROR_BUFFER_OVERRUN;
    ...
    if (PIF_PARREP_T17_VALID(ctldata)) {
...
__forceinline extern int pif_deparse(__lmem uint32_t *parrep, PIF_PKT_INFO_TYPE struct pif_pkt_info *pktinfo)
{
    ...
    ret = handle_tier_17(parrep, pktinfo, (uint32_t *)&pkt_byteoff, pkt_min_off);
    if (ret < 0)
        return ret;
    ret = handle_tier_16(parrep, pktinfo, (uint32_t *)&pkt_byteoff, pkt_min_off);
    if (ret < 0)
        return ret;
    ret = handle_tier_15(parrep, pktinfo, (uint32_t *)&pkt_byteoff, pkt_min_off);
    if (ret < 0)
        return ret;
    ret = handle_tier_14(parrep, pktinfo, (uint32_t *)&pkt_byteoff, pkt_min_off);
    if (ret < 0)
        return ret;
