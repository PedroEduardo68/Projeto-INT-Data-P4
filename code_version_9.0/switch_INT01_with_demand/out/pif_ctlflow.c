...
static int handle_egress_flow__condition_3(__lmem uint32_t *_pif_parrep)
{
    unsigned int pif_expression__condition_3_register_0;
    __lmem struct pif_parrep_ctldata *prdata = (__lmem struct pif_parrep_ctldata *)(_pif_parrep + PIF_PARREP_CTLDATA_OFF_LW);

#ifdef PIF_DEBUG
    __debug_label("pif_ctlflow_state_egress_flow__condition_3");
#endif

    //expression _condition_3: (valid(hopINT))
    {
    //subexpression 0: valid(hopINT)
    pif_expression__condition_3_register_0 = PIF_PARREP_hopINT_VALID(prdata);
    }
...
int pif_ctlflow_egress_flow(int *start_state, __lmem uint32_t *_pif_parrep, __mem __addr40 uint32_t *actbuf, unsigned int actbuf_off)
{
    __gpr int actlen, totlen = 0;
    __gpr int ret;
    int pif_ctlflow_state_egress_flow = PIF_CTLFLOW_STATE_egress_flow__condition_3;
    ...
    while (pif_ctlflow_state_egress_flow != PIF_CTLFLOW_STATE_egress_flow_DONE) {
        ...
        case PIF_CTLFLOW_STATE_egress_flow__condition_3:
            pif_ctlflow_state_egress_flow = handle_egress_flow__condition_3(_pif_parrep);
            continue;
